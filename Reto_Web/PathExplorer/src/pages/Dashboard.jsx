      ...
      {Array.from(new Array(3)).map((_, index) => (
        <Box key={index} sx={{ display: "flex", mb: 3, ml: 4 }}>
          <Skeleton
            ...
            width={16}
            height={16}
            sx={{ position: "absolute", left: 30, mt: 1.5 }}
          />
          <Skeleton
            variant="rounded"
            height={80}
...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (
        certStatsResult.status === "fulfilled" &&
        certStatsResult.value.data
      ) {
        const certStats = certStatsResult.value.data.reduce(
          (acc, cert) => {
      ...
      if (
        skillsDataResult.status === "fulfilled" &&
        skillsDataResult.value.data
      ) {
        ...
        const skillsWithStats = skillsDataResult.value.data.map((skill) => ({
          ...
          popularityPercentage:
            Math.round((skill.UserSkills?.length / totalCount) * 100) || 50,
          userCount: skill.UserSkills?.length || 0,
          projectCount: Math.floor(Math.random() * 10) + 1,
        }));

        setPopularSkills(
          skillsWithStats.length > 0 ? skillsWithStats : DEFAULT_SKILLS
        );
      }
      ...
      if (
        certificationsDataResult.status === "fulfilled" &&
        certificationsDataResult.value.data
      ) {
        const totalUsers = await supabase
          .from("User")
          .select("*", { count: "exact", head: true });
        ...
        const certsWithStats = certificationsDataResult.value.data
          .map((cert) => {
            const approvedCount =
              cert.UserCertifications?.filter((uc) => uc.status === "approved")
                .length || 0;
            return {
        ...
        setPopularCertifications(
          certsWithStats.length > 0 ? certsWithStats : DEFAULT_CERTIFICATIONS
        );
      }
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} lg={8}>
            ...
            <Box sx={{ mb: 3 }}>
              <DashboardTimeline
                items={timelineItems}
                profilePurple={theme.palette.primary.main}
                loading={timelineLoading}
              />
            </Box>
                      ...
                      >
                        ...
                        >
                          <Box>
                            ...
                            >
                              {skill.category} • {skill.userCount} professionals

...
const DashboardTimelineItem = ({
  ...
  const getIcon = () => {
    ...
    if (item.type === "Course")
      return <BookmarkBorderIcon sx={{ fontSize: 18 }} />;
    return <EmojiEventsIcon sx={{ fontSize: 18 }} />;
  };

  // Colores según el tipo usando Accenture colors
  const getTypeColor = () => {
    if (isProject) return theme.palette.accenture.colors.blue;
    if (item.type === "Course") return theme.palette.accenture.colors.orange;
                    ...
                    label={
                      item.status === "Completed" || item.status === "approved"
                        ? (item.type || "Certification")
                            .charAt(0)
                            .toUpperCase() +
                          (item.type || "Certification").slice(1)
                        : item.type || "Certification"
                    }
                    size="small"
                    sx={{
            ...
            >
              <Chip
                ...
                sx={{
                  ...
                  bgcolor:
                    item.status === "Completed" || item.status === "approved"
                      ? alpha(theme.palette.accenture.colors.green, 0.15)
                      : item.status === "In Progress"
                      ? alpha(theme.palette.accenture.colors.orange, 0.15)
...
const DashboardTimeline = ({
  items = [],
  profilePurple = theme.palette.accenture.colors.corePurple1,
  loading = false,
}) => {
              ...
              >
                ...
                <Paper
                  ...
                  sx={{
                    flex: 1,
                    p: { xs: 2, sm: 2.5 },
                    backgroundColor: theme.palette.accenture.colors.blue,
                    borderRadius: 2,
                    border: `1px solid ${alpha(profilePurple, 0.15)}`,
                    height: 80,
                    display: "flex",
